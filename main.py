...
class Processor():
    ...
    def load_model(self):
        ...
        self.epoch_from_trained_model = None

        if self.arg.weights:
            self.epoch_from_trained_model = int(self.arg.weights.split("-")[1].replace(".pt",""))
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # forward
            output, loss_distill = self.model(data, epoch)
            ...
            loss = self.loss(output, label) + l1 + loss_distill.sum() / label.size(0) / 10000
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if self.epoch_from_trained_model != None:
                    self.eval(
                        self.epoch_from_trained_model,
